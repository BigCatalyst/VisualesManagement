...
$(function(){
   ...
   // scroll down landing page
	...
	if(windowWidth > 1200){
		...
		var stickySidebar2 = $('#side-b');
		var mainCt2 = $('.main-content');
		if (stickySidebar2.length > 0) {
			var stickyHeight2 = stickySidebar2.height(),
			sidebarTop2 = stickySidebar2.offset().top;
		}
		// on scroll move the sidebar
		$(window).scroll(function () {
		  if (stickySidebar2.length > 0) {
		    ...
		    if (sidebarTop2 < scrollTop2) {
		      stickySidebar2.css('top', scrollTop2 - sidebarTop2+ 300);

		      // stop the sticky sidebar at the footer to avoid overlapping
		      var sidebarBottom2 = stickySidebar2.offset().top + stickyHeight2,
		        stickyStop2 = mainCt2.offset().top + mainCt2.height();
		      if (stickyStop2 < sidebarBottom2) {
		        var stopPosition2 = mainCt2.height() - stickyHeight2 + 5;
		        stickySidebar2.css('top', stopPosition2);
		      }
		    }
		    else {
		      stickySidebar2.css('top', '160px');
		    }
		  }
		});
		$(window).resize(function () {
		  if (stickySidebar2.length > 0) {
		    stickyHeight2 = stickySidebar2.height();
		  }
		});
