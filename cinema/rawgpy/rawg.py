...
class RAWG(metaclass=utils.Singleton):
    ...
    def search_request(self, query, num_results=5, additional_param="") -> dict:
        ...
        :rtype: dict
        """
        url = "https://api.rawg.io/api/games?key={key}&page_size={number}&search={query}".format(
            number=num_results, query=query, key=settings.RAWG_API_KEY)
    ...
    def game_request(self, slug, additional_param="") -> dict:
        ...
        :rtype: dict
        """
        url = "https://api.rawg.io/api/games/{slug}?key={key}".format(
            slug=slug, key=settings.RAWG_API_KEY)
        return self.get_request(url)

    def game_screenshots_request(self, slug) -> dict:
        """uses the :attr:`~rawgpy.RAWG.get_request` method to get a specific games json
        ...
        :rtype: dict
        """
        url = "https://api.rawg.io/api/games/{slug}/screenshots?key={key}".format(
            slug=slug, key=settings.RAWG_API_KEY)
        return self.get_request(url)
    ...
    def game_screenshots(self, game_pk, num_results=5) -> list:
        """searches for games

        :param game_pk: the slug of the game
        :type game_pk: str
        :param num_results: the amount of results, defaults to 5
        :type num_results: int, optional
        """
        search_res = self.game_screenshots_request(game_pk)
        results = [j["image"]
                   for j in search_res.get("results", {})]
        return results
    ...
    def create_collection(self, name, description) -> collection.Collection:
        ...
        return collection.Collection(self.post_request("https://rawg.io/api/collections", data))
