...
def verify_response_signature(sig_header, digest_header, date_header,
                              response_body):
    ...
    sig_params = dict(
        map(lambda param: re.match('([^=]+)="([^"]+)"', param).group(1, 2), re.split(',\s*', sig_header))
    )

    # Reconstruct the signing data
    digest_bytes = base64.b64encode(
        hashlib.sha256(response_body.encode()).digest())
    ...
    verify_key = ed25519.VerifyingKey(settings.KEYGEN_KEY.encode(),
                                      encoding='hex')

    # Verify the response signature
    try:
        verify_key.verify(sig_params['signature'], signing_data.encode(),
                          encoding='base64')
...
def validate_license_key(key):
    ...
    try:
        ...
        return data['meta']['valid'], data['meta']['constant'], data['meta'][
            'ts'], is_online
    ...
    # by pinging https://google.com, or https://api.keygen.sh/v1/ping.
        ...
        if cache_data != None:
            sig, digest, license_date, body = cache_data
